...
void SetRoomRotationUsingHead() {
	__try
	{
		s_render->SetRoomRotationUsingHead();
	}
	__except (true)
	{
		//avoid crashing Unity
...
void ClearRoomToWorldTransform() {
	__try
	{
		s_render->ClearRoomToWorldTransform();
	}
	__except (true)
	{
		//avoid crashing Unity
...
GetViewport(int eye) {
	...
	return osvr::renderkit::OSVR_ViewportDescription();
}
...
GetProjectionMatrix(int eye) {
	...
	return osvr::renderkit::OSVR_ProjectionMatrix();
}
